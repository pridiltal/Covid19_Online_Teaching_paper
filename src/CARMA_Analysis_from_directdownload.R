...
data_world <- read.csv( here::here("CARMAdatadirectdownload", "1Digital_lms_world.csv"))
d<- data_world %>% 
...
data_world <- read.csv( here::here("CARMAdatadirectdownload", "2mobile_phones_apps_world.csv"))
d<- data_world %>% 
  mutate_at(vars(-date), as.character)
...
keywords <-  c("Kolibri", "Rumie", "Ustad Mobile")
data_world <- read.csv( here::here( "CARMAdatadirectdownload", "3offline_functionality_world.csv"))
d<- data_world %>% 
  mutate_at(vars(-date), as.character)
...
keywords= c("Alison", "Canvas", "Coursera", "European Schoolnet Academy", 
            "EdX", "iCourse", "Future Learn", "Icourses", "TED-Ed Earth School",
            "Udemy", "XuetangX")
data_world <- read.csv( here::here( "CARMAdatadirectdownload", "4mooc_world.csv"))
...
keywords= c("ABRA", "British Council", "Byju’s", "Code It", 
            ...
            "Polyup", "Quizlet", "SDG Academy Library", "Siyavula",
            "Smart History", "youtube")
data_world <- read.csv( here::here( "CARMAdatadirectdownload", "5Self_directed_learning_world.csv"))
...
data_world <- read.csv( here::here( "CARMAdatadirectdownload", "6Mobile_reading_world.csv"))
...
keywords= c("Zoom", "Dingtalk", "Lark", "Hangouts Meet",
            "Teams", "Skype", "WhatsApp")
data_world <- read.csv( here::here( "CARMAdatadirectdownload", "7Collaboration_platforms_world.csv"))
...
data_world <- read.csv( here::here( "CARMAdatadirectdownload", "8Tools_for_teachers_world.csv"))
d<- data_world %>% 
  mutate_at(vars(-date), as.character)
...
data_world <- read.csv( here::here( "CARMAdatadirectdownload", "9External_repositories_DL_world.csv"))
...
data_world <- read.csv( here::here( "CARMAdatadirectdownload", "10Online_proctoring_world.csv"))
...
## ---- psychosocialSupport
keywords <- c("psychosocial support")
data_world <- read.csv( here::here( "CARMAdatadirectdownload", "psychosocial_support_world.csv"))
...
p1 <- covidcasesW1 |>  autoplot() +
  xlab("date") + 
  scale_color_brewer(palette="Dark2") +
  scale_x_date(limits = as.Date(c('2019-12-01','2024-04-30')), date_breaks = "6 month") +
  theme_bw()+
  theme(legend.position = "bottom", text = element_text(size = 14)) +
  ggtitle("(a)")+
  xlab("") 
